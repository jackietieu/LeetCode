    ...
    words.forEach((word, i) => {
        ...
        if (wordsSeparatedByLine[currentLine]) {
            if (currentWordLength + currentLineLength + 1 <= maxWidth) {
                wordsSeparatedByLine[currentLine] = wordsSeparatedByLine[currentLine] ? wordsSeparatedByLine[currentLine].concat(word) : [word];
                currentLineLength += currentWordLength + 1;
            } else if ((word.length + currentLineLength + 1 > maxWidth) || words[i + 1] && currentLineLength + currentWordLength + words[i + 1] >= maxWidth) {
                currentLine++;
...
function addSpaces(array, maxWidth, lastLine) {
    ...
    if (lastLine) {
        let lastLineJustified = array.reduce((result, word) => result ? result + ` ${word}` : word, '');
    ...
    if (array.length === 1) {
        ...
        return array[0] + " ".repeat(maxWidth - array[0].length - array[1].length) + array[1];
    }

    result = array.reduce((result, word, i) => 
